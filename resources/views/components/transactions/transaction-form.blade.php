...
<form method="POST" action="{{ $action }}" class="space-y-5">
    ...
    <fieldset>
        <legend class="mb-2 text-xs font-medium text-slate-600">Тип операции</legend>

        <div class="grid grid-cols-2 gap-2">
            <label class="cursor-pointer">
                <input type="radio" name="type" value="expense" class="peer sr-only"@checked($selectedType === 'expense')>
            ...
            <label class="cursor-pointer">
                <input type="radio" name="type" value="income" class="peer sr-only"@checked($selectedType === 'income')>
    ...
    <div>
        <label for="description" class="mb-1.5 block text-xs font-medium text-slate-600">
            Описание<span class="font-normal text-slate-400">необязательно</span>
        </label>
